...
const collections = (connection, dbVersionSupport) => {
    ...
    return {
        create: async function (config) {
            ...
            const { supports: serverAppliesDevaultVIT } = await dbVersionSupport.supportsServerSideDefaultVectorIndexType();
            if (!serverAppliesDevaultVIT && vectorsConfig) {
                for (const v of Object.values(vectorsConfig)) {
                    if (!v.vectorIndexType)
                        v.vectorIndexType = 'hnsw';
                }
            }
            ...
            await new index_js_1.ClassCreator(connection).withClass(schema).do();
            return (0, index_js_2.default)(connection, name, dbVersionSupport);
        },
        createFromSchema: async function (config) {
            const { class: name } = await new index_js_1.ClassCreator(connection).withClass(config).do();
            return (0, index_js_2.default)(connection, name, dbVersionSupport);
        },
        createFromJson: async (schemaJson) => {
            const { class: name } = await connection.postReturn('/schema', schemaJson);
            return (0, index_js_2.default)(connection, name, dbVersionSupport);
        },
        ...
        exportToJson: (name) => connection.get(`/schema/${name}`, true),
        listAll: listAll,
